...
export default class AddressTypeaheadSearch extends LightningElement {
    @api label;
    @api selection = [];
    @api placeholder = '';
    ...
    handleResultClick(event) {
		...
		if (DEBUG_MODE) {
		    console.debug('handleResultClick invoked');
		}
        ...
        this.searchTerm = '';
        this.searchResults = [];
        let addressSelected = selectedItem.title;
        // Update search results
        // Notify parent components that selection has changed
        this.dispatchEvent(new CustomEvent('selectionchange', {
            detail: addressSelected
        }));
    }
    ...
    get getInputClass() {
        let css =
            'combobox-text slds-input slds-combobox__input has-custom-height ' +
    ...
    get getSearchIconClass() {
        let css = 'slds-combobox__input-entity-icon slds-icon_container ';
        if (!this.isMultiEntry) {
    ...
    get getSelectIconClass() {
        return (
            'slds-combobox__input-entity-icon slds-icon_container ' +
            (this.hasSelection() ? '' : 'slds-hide')
        );
    }
